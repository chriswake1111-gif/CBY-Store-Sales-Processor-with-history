...
import { FileSpreadsheet, Save, Trash2, X, Upload, CheckCircle2, ArrowRight, User, Stethoscope, RefreshCcw } from 'lucide-react';
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-gray-500 leading-relaxed px-1">
                                <strong>說明：</strong> 上傳後請切換至「欄位對應」指定資料欄位。不同職位請分別上傳對應的範本。
                            </p>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'MAPPING' && (
                    <div className="space-y-4">
                        <div className="bg-blue-50 p-3 rounded border border-blue-100 text-blue-800 text-xs mb-4">
                            設定系統資料要填入 Excel 的哪一欄 (例如 A, B)。未填寫欄位代號的資料將不會被匯出。
                        </div>
                        ...
                        {activeTypeId === TEMPLATE_IDS.REPURCHASE ? (
                            <div className="space-y-1">
                                ...
                                {renderInput('回購點數', 'repurchasePoints', 'G')}
                                {renderInput('原開發者', 'originalDeveloper', 'H')}
                                {renderInput('開發點數', 'devPoints', 'I')}
                            </div>
                        ) : (
                            <div className="space-y-1">
                                {renderInput('分類', 'category', 'A')}
                                ...
                                {renderInput('備註 (狀態)', 'note', 'H')}
                                {renderInput(activeTypeId === TEMPLATE_IDS.PHARMACIST ? '藥師點數' : '計算點數', 'points', 'I')}
                            </div>
                        )}
